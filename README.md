...
ffmpeg 已配置为内置，在打包时会自动包含。用户无需单独安装。

如果开发时需要手动下载 ffmpeg：
1. 在有代理的电脑上下载 ffmpeg Windows x86_64 版本
2. 放入 `src-tauri/binaries/ffmpeg-x86_64-pc-windows-msvc.exe`
3. 验证：`file ffmpeg-x86_64-pc-windows-msvc.exe` 应显示 `ELF 64-bit`
...
### Q: 提示 "未找到 ffmpeg"？
A: 请确保 `src-tauri/binaries/ffmpeg-x86_64-pc-windows-msvc.exe` 文件存在。打包时会自动包含。
...
│   │   └── main.rs              # Rust 命令实现
│   ├── binaries/
│   │   └── ffmpeg-x86_64-pc-windows-msvc.exe  # 内置 ffmpeg
│   ├── Cargo.toml
│   ├── tauri.conf.json
│   └── capabilities/
├── package.json
